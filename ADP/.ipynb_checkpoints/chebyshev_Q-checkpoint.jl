...
returns the action that produces the maximum reward  
"""
function argmax_ind_A_discrete(Q,s)
    V = broadcast(x -> evaluate_Q_discrete(Q,s,x), Q.A)
    a = argmax(V)
    return a
end 
...
"""
function init_Q_discrete(a,b,m,N_actions,grid,extrapolate)
    polynomials = []
    for i in 1:N_actions
        push!(polynomials, OLS_chebyshev.init_polynomial(a,b,m,extrapolate))
    end 
    return Q_discrete(collect(1:N_actions), grid, a, b, polynomials, 
                        evaluate_Q_discrete, 
                        argmax_A_discrete, 
                        argmax_ind_A_discrete, 
                        max_A_discrete)
end
