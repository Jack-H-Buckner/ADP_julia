...
function init(T!::Function,
    ...
    bellmanIntermidiate = broadcast(i -> BellmanOpperators.init_bellmanIntermidiate(dims_x,dims_y,m_Quad_x,m_Quad_y),1:nthread)
...
function solve_observed_parallel(kalmanFilterSolver)
    tol = 10^-4
    max_iter = 3*10^2
    test = tol+1.0
